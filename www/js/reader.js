...
const Reader = (() => {
  ...
  function applyPremiumLimits() {
    ...
    document.querySelectorAll('.groupBtn, .landscapeGroupBtn').forEach(btn => {
      ...
      const visibleLabel = btn.classList.contains('landscapeGroupBtn') ? String(n) : label;
      btn.innerHTML = locked ? `<span class="lockIcon">&#128274;</span> ${visibleLabel}` : visibleLabel;
    });
  ...
  function setGroupSize(n) {
    if (!Storage.isPremium() && n > FREE_LIMITS.MAX_WORDS_PER_GROUP) {
      Premium.show(() => { applyPremiumLimits(); setGroupSize(n); });
      return;
    }
    engine.wordsPerGroup = n;
    document.querySelectorAll('.groupBtn, .landscapeGroupBtn').forEach(b => {
  ...
  function syncLandscapeMode() {
    ...
    const compactLandscape = viewportLandscape && viewportHeight <= 700;
    const active = (landscapeQuery.matches || compactLandscape) && !els.readerScreen.classList.contains('hidden');
  ...
  function initEvents() {
    ...
    landscapeQuery = window.matchMedia('(orientation: landscape) and (max-height: 700px)');
    ...
    window.addEventListener('orientationchange', () => setTimeout(syncLandscapeMode, 80));
    if (window.visualViewport) {
      window.visualViewport.addEventListener('resize', syncLandscapeMode);
    }
  }
