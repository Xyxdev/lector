...
// Busca un archivo en el zip probando primero la ruta decodificada
// (la mayoría de los EPUB guardan el archivo real con el nombre "tal cual",
// incluso si el .opf referencia un href URL-encoded como "cap%C3%ADtulo.xhtml")
// y si falla, prueba la ruta cruda por si el archivo en el zip sí está
// almacenado con el nombre encodeado.
function findZipFile(zip, path) {
  let decoded = path;
  try { decoded = decodeURIComponent(path); } catch (e) { /* path no es válido como URI, lo dejamos igual */ }
  return zip.file(decoded) || zip.file(path);
}
...
async function parseEpub(file) {
  ...
  const titleFromMeta = titleEl ? titleEl.textContent.trim() : '';
  const bookTitle = titleFromMeta || file.name.replace(/\.epub$/i, '');
  ...
  for (const id of spineIds) {
    ...
    const zf = findZipFile(zip, fullPath);
    if (!zf) continue;
