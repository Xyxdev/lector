...
class RsvpEngine {
  constructor({ chapters, onWord, onDone }) {
    ...
    this.flowCount = 0;
    // Tope opcional de ppm EFECTIVO (no solo basePPM). Lo usa reader.js
    // para que el modo adaptativo no pueda acelerar la lectura por encima
    // del límite gratis — sin que este motor necesite saber qué es
    // "premium", solo respeta un número si se lo pasan desde afuera.
  ...
  delayForGroup(groupRaw) {
    ...
    delay *= this.adaptMult;
    // Si hay un tope de ppm efectivo configurado (versión gratis), nunca
    // dejamos que el delay resulte en una velocidad mayor a ese tope,
    // sin importar cuánto haya acelerado el modo adaptativo.
    if (this.maxEffectivePPM) {
      const minDelayPerWord = 60000 / this.maxEffectivePPM;
      const minDelay = minDelayPerWord * groupRaw.length;
  ...
  effectivePPM() {
    const raw = this.basePPM / this.adaptMult;
  ...
  seekToWord(pos) {
    ...
    // Alineamos al inicio de grupo más cercano hacia atrás, para que el
    // agrupamiento de palabras (2-3) no quede descuadrado tras un salto.
  ...
  /** Reinicia el estado adaptativo (multiplicador de ritmo, historial de
   *  pausas). Se usa al "empezar de nuevo" un libro, para no heredar el
   *  ritmo acelerado/desacelerado de una sesión de lectura anterior. */
  resetAdaptiveState() {
    this.adaptMult = 1.0;
    this.pauseEvents = [];
    this.flowCount = 0;
